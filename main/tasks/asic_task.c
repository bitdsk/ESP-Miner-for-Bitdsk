    ...
    while (1) {
        if(GLOBAL_STATE->ASIC_jobs_queue.count>0){
            ...
            if (next_bm_job->pool_diff != GLOBAL_STATE->stratum_difficulty) {
                ESP_LOGW(TAG, "job_diff:%lu != stratum_difficulty:%lu", next_bm_job->pool_diff ,GLOBAL_STATE->stratum_difficulty);
                ...
                free(next_bm_job->jobid);
                free(next_bm_job->extranonce2);
                free(next_bm_job);
                continue;
            }
